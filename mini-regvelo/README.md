...
学习文献：*RegVelo: Gene-regulatory-informed dynamics of single cells*, Cell 189,
3773–3800 (2026)，DOI: https://doi.org/10.1016/j.cell.2026.04.022。
# 1. 数学原理

这个项目的核心问题是：

**已知很多细胞在某一状态下的 RNA 信息，能否反推出它们背后的动态演化规律？**

RegVelo 可以看作一个高维动力系统的反问题。

## 1.1 实验给了我们什么？

假设共有 $N$ 个细胞，并选取 $G$ 个基因进行分析。

实验得到两个矩阵：

$$
U_{\mathrm{obs}}\in\mathbb{R}^{N\times G}
$$

和

$$
S_{\mathrm{obs}}\in\mathbb{R}^{N\times G}.
$$

其中：

- $U_{\mathrm{obs}}$：unspliced RNA 丰度；
- $S_{\mathrm{obs}}$：spliced RNA 丰度；
- 每一行对应一个细胞；
- 每一列对应一个基因。
...
=
\begin{bmatrix}
u_{11} & u_{12} & \cdots & u_{1G} \\
u_{21} & u_{22} & \cdots & u_{2G} \\
\vdots & \vdots & \ddots & \vdots \\
u_{N1} & u_{N2} & \cdots & u_{NG}
\end{bmatrix}
```

其中 $u_{ng}$ 表示第 $n$ 个细胞中第 $g$ 个基因的 unspliced RNA 丰度，而s<sub>ng</sub>表示同一个细胞、同一个基因的 spliced RNA 丰度。

因此，对于每一个 cell-gene pair，我们观测到的是：

$$
(u_{ng},s_{ng}).
$$

这些数据是很多不同细胞的静态快照，并不是同一个细胞随时间连续测量得到的时间序列。


## 1.2 先验基因调控网络

除了 RNA 丰度数据外，RegVelo 还可以使用一个先验基因调控网络（prior GRN）。设分析中共有 \(G\) 个基因，先验网络可以表示为一个二值矩阵：
...
[g_{gj}]
\in
\left\{0,1\right\}^{G\times G}.
```

其中，矩阵的**第 \(g\) 行表示目标基因（target gene）**，**第 \(j\) 列表示调控基因（regulator）**。
...
=
\begin{cases}
1, & \text{先验网络中包含调控关系 } j\rightarrow g,\\
0, & \text{先验网络中未包含该调控关系}.
\end{cases}
```

例如，若分析三个基因 \(A,B,C\)，先验网络为：
...
需要注意，它只是一个**二值的网络骨架**。矩阵元为 1 只表示先验信息中记录了这条 regulator-target 连接，并不包含调控强度，也不区分激活或抑制。

真正的调控方向和强度由模型学习得到的权重矩阵 \(W\) 表示。
...
这个矩阵只提供网络的结构信息，并不直接告诉我们调控是激活还是抑制，也不直接给出调控强度。

真正的调控权重由模型学习得到的矩阵 $W$ 表示：
...
对于第 g 个基因，unspliced RNA 和 spliced RNA 的动力学分别为：

```math
\frac{d u_g}{d t}
=
\alpha_g(t)-\beta_g u_g(t)
```

```math
\frac{d s_g}{d t}
=
\beta_g u_g(t)-\gamma_g s_g(t)
```

其中：

- α<sub>g</sub>(t)：第 g 个基因的转录速率；
- β<sub>g</sub>：第 g 个基因的剪接速率；
- γ<sub>g</sub>：第 g 个基因的降解速率。

RegVelo 的关键在于：转录速率 α<sub>g</sub>(t) 不是一个固定常数，而是由其他基因当前的表达状态通过基因调控网络决定：
...
\mathrm{softplus}
\left(
\sum_j W_{gj}s_j(t)+b_g
\right)
```

其中，W<sub>gj</sub> 表示调控基因 j 对目标基因 g 的调控权重，b<sub>g</sub> 表示该基因的基础转录项。
...
\vec{\beta}\odot\vec{u}
-
\vec{\gamma}\odot\vec{s}
```

其中，符号 ⊙ 表示对应元素逐项相乘。

由于矩阵 W 中的非零元素将不同基因连接起来，一个基因的 spliced RNA 丰度可以影响另一个基因的转录速率。因此，各基因的动力学不再彼此独立，而是共同组成一个高维耦合动力系统。
...
以及每个细胞在动态过程中的 latent time。

latent time（潜在时间）是模型根据每个细胞的 unspliced 和 spliced RNA 状态推断出的一个隐藏时间坐标，用来表示该细胞在动态生物过程中的相对位置。由于单细胞 RNA 测序得到的是许多不同细胞的静态快照，而不是同一个细胞随真实时间连续变化的数据，因此模型必须先估计“这个细胞大概处在轨迹的哪里”，才能把观测到的 RNA 状态与 ODE 轨迹中的某个位置对应起来。RegVelo 中的 latent time 因此更接近“动力学进程”或“发育进度”，而不是分钟、小时这样的真实物理时间。


我们的目的是构建模型，反推出一套动力学参数，使 ODE 生成的

$$
(U_{\mathrm{pred}},S_{\mathrm{pred}})
$$
...
```mermaid
flowchart TD
    A["输入数据<br/>U_obs, S_obs"] --> B["初始化参数<br/>W, b, β, γ"]
    P["先验网络<br/>G_prior"] --> C["约束 W 的学习"]
    ...
    E --> F["求解 ODE"]
    F --> G["得到预测状态<br/>U_pred, S_pred"]

    G --> H["与观测数据比较"]
    H --> I["计算 loss / ELBO"]

    I --> J["更新参数<br/>W, b, β, γ<br/>以及 latent time 模块参数"]
...
当动力学参数确定后，RNA velocity 可以直接由动力学方程得到：

$$
\vec{v}=\frac{d\vec{s}}{dt}=\vec{\beta}\odot\vec{u}-\vec{\gamma}\odot\vec{s}
$$


 velocity 不是由另一个独立神经网络预测出来的，是已经学习到的动力系统在当前状态下的时间导数。

反复修改调控矩阵 $W$ 并重新求解 ODE，就可以模拟基因调控扰动后系统动力学的变化。
